...
use App\Models\adm_userdata;
use DB;

class AdmUserdataRepository {

    public function getAllData(){
        return adm_userdata::where('isflag',1)->get();
    }
    ...
    public function getTenData($query_name,$query_status) {
        try {
            ...
            $adm_string = adm_userdata::where('usd_name','like', '%'.$query_name.'%');
    ...
    public function getDataByAccountPwd($usd_account,$usd_pwd) {
        return adm_userdata::where('usd_status',1)->where('usd_account',$usd_account)->where('usd_pwd',$usd_pwd)->where('isflag',1)->get();
    }
    ...
    public function getDataBySerno($usd_serno) {
        return adm_userdata::where('usd_serno',$usd_serno)->where('isflag',1)->get();
    }
    ...
    public function getDataByName($usd_name) {
        return adm_userdata::where('usd_status',1)->where('usd_admin',1)->where('usd_name',$usd_name)->where('isflag',1)->get();
    }

    public function insertMoreThanOne($arraydata){
        try {
            DB::beginTransaction();
            $result = adm_userdata::insert($arraydata);
    ...
    public function create(array $arraydata) {
        try {
            ...
            $data = adm_userdata::where('usd_account',$arraydata['usd_account'])->where('isflag',1)->get();
            if(count($data) > 0){
                return false;
            }
            DB::beginTransaction();
            ...
            $result = adm_userdata::insert($savedata);
            if($result){
                DB::commit();
                return true;
            }
    ...
    public function update(array $arraydata) {
        try {
            ...
            $result = adm_userdata::where('usd_serno',$arraydata['serno'])->update($savedata);
            if($result){
                DB::commit();
                return true;
            }
    ...
    public function delete($usd_serno) {
        try {
            ...
            $result = adm_userdata::where('usd_serno',$usd_serno)->update($savedata);
            if($result){
                DB::commit();
                return true;
            }
